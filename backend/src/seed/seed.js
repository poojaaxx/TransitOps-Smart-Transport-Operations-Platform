...
const { ROLES, VEHICLE_STATUS, DRIVER_STATUS, TRIP_STATUS, MAINTENANCE_STATUS } = require('../utils/constants');
...
const seed = async () => {
  ...
  const users = await User.create([
    { name: 'Frank Mercer', email: 'fleetmanager@transitops.demo', password: DEMO_PASSWORD, role: ROLES.FLEET_MANAGER },
    { name: 'Sara Lin', email: 'safety@transitops.demo', password: DEMO_PASSWORD, role: ROLES.SAFETY_OFFICER },
    { name: 'Amara Chen', email: 'finance@transitops.demo', password: DEMO_PASSWORD, role: ROLES.FINANCIAL_ANALYST },
    { name: 'Diego Ruiz', email: 'driver@transitops.demo', password: DEMO_PASSWORD, role: ROLES.DRIVER },
  ]);
  ...
  const [truck1, truck2, van1, van2, bus1, car1, trailer1] = vehicles;
  ...
  await User.findOneAndUpdate({ email: 'driver@transitops.demo' }, { driver: driver1._id });

  console.log('Creating trips (one per lifecycle state)...');
  await Trip.create([
    {
      source: 'Chicago, IL', destination: 'Detroit, MI', vehicle: truck2._id, driver: driver1._id,
    ...
    {
      source: 'Boston, MA', destination: 'New York, NY', vehicle: van1._id, driver: driver2._id,
      cargoWeightKg: 900, plannedDistanceKm: 340, revenue: 900, status: TRIP_STATUS.DRAFT,
      createdBy: users[0]._id,
    },
    {
      source: 'Dallas, TX', destination: 'Houston, TX', vehicle: bus1._id, driver: driver5._id,
    ...
    {
      source: 'Seattle, WA', destination: 'Portland, OR', vehicle: trailer1._id, driver: driver2._id,
    ...
    {
      source: 'Miami, FL', destination: 'Orlando, FL', vehicle: van1._id, driver: driver3._id,
  ...
  await Maintenance.create([
    { vehicle: van2._id, description: 'Transmission repair', cost: 1800, startDate: daysFromNow(-3), status: MAINTENANCE_STATUS.ACTIVE, createdBy: users[0]._id },
    { vehicle: truck1._id, description: 'Routine service + oil change', cost: 320, startDate: daysFromNow(-30), endDate: daysFromNow(-28), status: MAINTENANCE_STATUS.COMPLETED, createdBy: users[0]._id },
    { vehicle: bus1._id, description: 'Brake pad replacement', cost: 540, startDate: daysFromNow(-20), endDate: daysFromNow(-19), status: MAINTENANCE_STATUS.COMPLETED, createdBy: users[0]._id },
  ]);

  console.log('Creating fuel logs...');
  await FuelLog.create([
    { vehicle: truck2._id, liters: 180, cost: 320, date: daysFromNow(-6), odometerKm: 87500, createdBy: users[0]._id },
    { vehicle: truck2._id, liters: 150, cost: 275, date: daysFromNow(-1), odometerKm: 88000, createdBy: users[0]._id },
    { vehicle: bus1._id, liters: 90, cost: 165, date: daysFromNow(-6), odometerKm: 62800, createdBy: users[0]._id },
    { vehicle: trailer1._id, liters: 210, cost: 390, date: daysFromNow(-12), odometerKm: 14700, createdBy: users[0]._id },
    { vehicle: van1._id, liters: 40, cost: 78, date: daysFromNow(-15), odometerKm: 20800, createdBy: users[0]._id },
  ]);

  console.log('Creating expenses...');
  await Expense.create([
    { vehicle: truck2._id, category: 'Toll', amount: 45, date: daysFromNow(-6), notes: 'I-94 tolls', createdBy: users[0]._id },
    { vehicle: bus1._id, category: 'Permit', amount: 120, date: daysFromNow(-20), notes: 'Annual route permit', createdBy: users[0]._id },
    { vehicle: trailer1._id, category: 'Insurance', amount: 500, date: daysFromNow(-30), createdBy: users[0]._id },
    { category: 'Other', amount: 75, date: daysFromNow(-2), notes: 'Office fleet-tracking software subscription', createdBy: users[0]._id },
  ]);
